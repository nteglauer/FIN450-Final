...
```{r Movie Revenue Trends over Time}
...
recipe_pipeline <- recipes::recipe(Worldwide ~ Budget + Distributor + `Earliest Release Date` + MPAA + `Run Time (Mins)`, data = df_train) %>%
  # step_rm(date) %>%
  recipes::prep()

train_baked <- recipes::bake(recipe_pipeline, df_train)

recipe_pipeline <- recipes::recipe(Worldwide ~ Budget + Distributor + `Earliest Release Date` + MPAA + `Run Time (Mins)`, data = df_test) %>%
  # step_rm(date) %>%
  recipes::prep()
...
model <- parsnip::decision_tree(mode = "regression") %>%
  ...
  parsnip::fit(Worldwide ~ Budget + Distributor + `Earliest Release Date` + MPAA + `Run Time (Mins)`, data = train_baked)
model
