...
```{r Regression Tree Analysis, echo=FALSE, warning=FALSE, include=FALSE}
...
```{r Regression Tree Plots Importance, echo=FALSE}
...
```{r Regression Tree Plots Tree, echo=FALSE}
...
```{r error analysis, include-FALSE, echo=FALSE}
...
# Visual comparisons (e.g., boxplots)
ggplot(merged_df, aes(x = EstimateCategory, y = Budget)) +
  geom_boxplot() +
...
# Extract metrics from both tables
metrics_original <- metrics_table %>% as.data.frame()
metrics_new <- metrics_table_2 %>% as.data.frame()

# Combine metrics into a single data frame
combined_metrics <- bind_rows(
  mutate(metrics_original, Model = "Original"),
  mutate(metrics_new, Model = "New")
)
...
ggplot(combined_metrics_long, aes(x = Model, y = Value, fill = Metric)) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  labs(title = "Comparison of Model Metrics",
       subtitle = "Original vs. New Model",
       x = "Model",
       y = "Metric Value") +
  theme_minimal() +
