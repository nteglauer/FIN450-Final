...
budget <- df %>% 
  ...
  labs(title = "Average Revenue by Budget",
       x = "Length Category",
       y = "Average Revenue")
...
Over time, the run time of films has slowly risen. Films also tend to do better the longer they are. However, the difference isn't as drastic between short and medium length films. Films were split into the following categories:
...
length_category_plot <- ggplot(length, aes(x = Length_Category, y = AvgRev, fill = Length_Category)) +
  ...
  labs(title = "Average Revenue by Length Category",
       x = "Length Category",
       y = "Average Revenue") +
  theme_minimal() +
  theme(legend.position = "none")
